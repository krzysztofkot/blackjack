...
export const UIController = {
    ...
    toggleDoubleBtn: () =>
        DOMselectors.doubleBtn.classList.toggle(DOMstrings.btnToggleClass),
    ...
    initCash: (cash) => DOMselectors.totalCash.textContent = cash,
    ...
    generateCardUI: (playerName, obj, allCards) => {
        ...
        if (allCards.length === 1) {
            ...
            cardContainer.querySelectorAll('.card').forEach((el) => el.classList.replace(`card--${allCards.length - 1}`, `card--${allCards.length}`));
            outerDiv.classList.add(`card--${allCards.length}`);
        ...
        } else { setTimeout(rotate, 500); }

    },

    rotateCroupierCard: () => {
        const firstCard = DOMselectors.croupierContainer.querySelector('.card:nth-child(2)');
    ...
    clearPoints: () => {
        const allPoints = document.querySelectorAll(`[class^='${DOMstrings.pointsPartial}']`);
    ...
    toggleDisplayPoints: (player) => {
        document.querySelector(`.points__${player}`).classList.toggle(DOMstrings.displayElement);
    },

    removeDisplayPoints: (player) => {
        document.querySelector(`.points__${player}`).classList.add(DOMstrings.displayElement);
    },

    togglePopup: () => DOMselectors.highscores.classList.toggle('display'),

    toggleStartAgainWindow: () => DOMselectors.popupPlayAgain.classList.toggle('display'),
